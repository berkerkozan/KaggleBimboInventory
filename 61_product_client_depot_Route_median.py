...
prod_median_tab2 = df_train.groupby(['ProductId', 'ClientId', 'DepotId', 'RouteId']).agg({'AdjDemand': np.median})
...
df_test['Demanda_uni_equil'] = df_test[['ProductId', 'ClientId', 'DepotId', 'RouteId']]. \
    apply(lambda x: gen_output(x), axis=1)
df_submit = df_test[['id', 'Demanda_uni_equil']]
df_submit = df_submit.set_index('id')

df_submit = df_submit.fillna(2).round(0).astype(int)
df_submit.to_csv('../output/naive_product_client_depot_route_median.csv')